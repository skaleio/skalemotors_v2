...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!content) return jsonResponse(400, { ok: false, error: "El contenido es requerido" });
  if (!platforms.length) return jsonResponse(400, { ok: false, error: "Selecciona al menos una cuenta" });

  if (scope === "org" && !canPublishOrg(auth.role)) {
    return jsonResponse(403, { ok: false, error: "No puedes publicar en cuentas de la automotora" });
  }
  ...
  if (accErr) return jsonResponse(500, { ok: false, error: accErr.message });
  if (!allowedAccounts?.length || allowedAccounts.length !== accountIds.length) {
    return jsonResponse(403, { ok: false, error: "Una o más cuentas no están autorizadas" });
  }
  ...
  if (insertErr) return jsonResponse(500, { ok: false, error: insertErr.message });
