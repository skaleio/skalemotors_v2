-- ============================================================================
-- Fix: "column reference 'sales_count' is ambiguous" en agg_by_user.
-- Se califica con el alias t. y se agrupa por t.user_id. Resto idéntico a v5.
-- ============================================================================
...
BEGIN
  ...
  agg_by_user AS (
    SELECT t.user_id AS user_id,
           SUM(t.sales_count)::bigint    AS user_sales_count,
           SUM(t.total_amount)::numeric  AS user_total_amount,
           SUM(t.total_margin)::numeric  AS user_total_margin
    FROM (
  ...
  rows_users AS (
    SELECT
      ...
      su.branch_id,
      COALESCE(a.user_sales_count, 0)::bigint   AS sales_count,
      COALESCE(a.user_total_amount, 0)::numeric AS total_amount,
      COALESCE(a.user_total_margin, 0)::numeric AS total_margin,
  ...
  rows_orphan_users AS (
    SELECT
      ...
      u.branch_id,
      a.user_sales_count   AS sales_count,
      a.user_total_amount  AS total_amount,
      a.user_total_margin  AS total_margin,
      TRUE AS is_linked_user
    FROM agg_by_user a
