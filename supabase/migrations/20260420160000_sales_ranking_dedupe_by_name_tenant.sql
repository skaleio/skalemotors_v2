-- ============================================================================
-- Ranking v5: dedupe staff ↔ user más robusto.
--
-- Problema de v4: exigía mismo branch_id entre user y staff. Si el admin crea
-- el staff con branch_id NULL (o el user cambia de sucursal), el dedupe se
-- rompía y el vendedor aparecía 2 veces.
--
-- Nueva regla: match por (tenant_id, lower(trim(full_name))). El branch deja
-- de ser requisito. Si hay >1 user con el mismo nombre (ambigüedad), se
-- prefiere el que comparte branch con el staff; si tampoco, el más antiguo.
-- ============================================================================
...
BEGIN
  ...
  RETURN QUERY
  WITH
  -- Dedupe: match relajado por tenant + nombre normalizado.
  -- DISTINCT ON (staff_id) con ORDER BY prefiere el user con mismo branch
  -- (si existe) y luego el más antiguo.
  staff_user_map AS (
    ...
    ORDER BY
      s.id,
      (u.branch_id IS NOT DISTINCT FROM s.branch_id) DESC,  -- prefer same branch
      u.created_at ASC
  ),
  closed_leads AS (
    SELECT
...
COMMENT ON FUNCTION public.get_sales_ranking(date, date, uuid) IS
  'Ranking v5: dedupe user↔staff por (tenant, nombre normalizado). '
  'Branch deja de ser requisito; se usa como desempate si hay ambigüedad.';

-- ----------------------------------------------------------------------------
-- Helper de diagnóstico: ver el mapa de dedupe actual y los huérfanos.
-- Úsalo así:  SELECT * FROM public.debug_ranking_dedupe();
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.debug_ranking_dedupe()
RETURNS TABLE (
  kind text,
  staff_id uuid,
  staff_name text,
...
AS $$
  WITH mapped AS (
    SELECT DISTINCT ON (s.id)
      s.id  AS staff_id, s.full_name AS staff_name, s.branch_id AS staff_branch,
      u.id  AS user_id,  u.full_name AS user_name,  u.branch_id AS user_branch
    FROM public.branch_sales_staff s
    JOIN public.users u
      ON u.tenant_id = s.tenant_id
     AND u.role = 'vendedor'
  ...
  SELECT 'matched'::text, staff_id, staff_name, staff_branch, user_id, user_name, user_branch
  FROM mapped
  UNION ALL
  SELECT 'staff_without_user'::text, s.id, s.full_name, s.branch_id, NULL::uuid, NULL::text, NULL::uuid
  FROM public.branch_sales_staff s
  WHERE s.tenant_id = public.current_tenant_id()
    AND s.is_active = TRUE
    AND NOT EXISTS (SELECT 1 FROM mapped m WHERE m.staff_id = s.id)
  UNION ALL
  SELECT 'user_without_staff'::text, NULL::uuid, NULL::text, NULL::uuid, u.id, u.full_name, u.branch_id
  FROM public.users u
  WHERE u.tenant_id = public.current_tenant_id()
    AND u.role = 'vendedor'
    AND u.is_active = TRUE
    AND NOT EXISTS (SELECT 1 FROM mapped m WHERE m.user_id = u.id);
$$;

GRANT EXECUTE ON FUNCTION public.debug_ranking_dedupe() TO authenticated;
