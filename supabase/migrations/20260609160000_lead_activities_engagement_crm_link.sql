-- lead_activities faltaba en prod (rompía get_seller_engagement_metrics).
-- Enlaza métricas CRM (user) con plantilla staff vía staff_id en user_rows.
...
CREATE TABLE IF NOT EXISTS public.lead_activities (
  ...
  type text NOT NULL CHECK (
    type IN ('llamada', 'email', 'whatsapp', 'reunion', 'test_drive', 'cotizacion', 'nota')
  ),
  subject text,
  description text,
  outcome text,
...
COMMENT ON TABLE public.lead_activities IS
  'Actividades registradas en el CRM por lead (llamadas, WhatsApp, reuniones, etc.).';
...
DROP POLICY IF EXISTS tenant_restrict_lead_activities ON public.lead_activities;
CREATE POLICY tenant_restrict_lead_activities ON public.lead_activities
...
-- Recrear RPC: user_rows incluye staff_id para tarjetas de Vendedores.
...
COMMENT ON FUNCTION public.get_seller_engagement_metrics(uuid, integer, integer) IS
  ...
  'staff_id en filas de usuario CRM enlaza plantilla branch_sales_staff con métricas reales.';
