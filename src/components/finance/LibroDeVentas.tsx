...
// Una "hoja" del libro: la tabla con totales para un conjunto de ventas
// (todas, o las de una sucursal). Los totales se calculan sobre `rows`.
function LibroTable({
  rows,
  socioNombres,
}: {
...
export function LibroDeVentas() {
  ...
  // Una hoja por sucursal (más la hoja "Todas"). Las ventas sin sucursal caen
  // en "Sin sucursal". Si hay una sola sucursal, no mostramos pestañas.
  const hojas = useMemo(() => {
    const rows = data?.rows ?? [];
    const map = new Map<string, LibroVentaRow[]>();
    for (const r of rows) {
      const key = branchName(r);
  ...
  const multiSucursal = hojas.length > 1;
      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <Tabs defaultValue="__all__" className="w-full">
            <TabsList className="mx-4 mt-4 flex-wrap justify-start">
            ...
            <TabsContent value="__all__" className="mt-3">
              <LibroTable rows={data.rows} socioNombres={socioNombres} />
            </TabsContent>
            {hojas.map(([name, rows]) => (
              <TabsContent key={name} value={name} className="mt-3">
                <LibroTable rows={rows} socioNombres={socioNombres} />
              </TabsContent>
            ))}
          </Tabs>
        ) : (
